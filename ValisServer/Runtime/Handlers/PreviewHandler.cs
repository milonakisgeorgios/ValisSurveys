...
{
    public class PreviewHandler : SurveyHttpHandler
    {
        /// <summary>
        ...
        protected override void ProcessRequestImplementation()
        {
            ...
            if(RuntimeSession.IsRessurected)
            {
                ...
                if (!string.IsNullOrWhiteSpace(Request.Params["frmCurrentPage"]))
                {
                    ...
                    {
                        throw new VLException("PreviewHandler::ProcessRequestImplementation -> PreviousPage == null");
                    }
                }
                #endregion
        ...
        void ShowSurvey()
        {
            ...
            //FORM ATTRIBUTES
            {
                var action = string.Format("{0}{1}", Utility.GetSurveyPreviewURL(Survey), RuntimeSession.SessionId.ToString());
            ...
            if (CurrentPage == null)
            {
                if (Navigation == NavigationDirections.Next)
                {
                    if (CurrentPageType == PageType.Disqualification)
                    {
                        ...
                        SurveyManager.DeleteSession(this.RuntimeSession);
                    }
                    else
                    {
