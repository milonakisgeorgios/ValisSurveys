...
{
    public class GetDraftMessages : WebApiHandler
    {
        ...
        protected override void ProcessGetRequestWrapped(Valis.Core.VLAccessToken accessToken, HttpContext context)
        {
            try
            {
                ...
                int totalRecords = 0;
                var items = manager.GetDraftMessages(collectorId, pageIndex, pageSize, ref totalRecords, string.Empty);
                ...
                var rows = items.Select(c => new
                {
                    ...
                    c.IsContentOK,
                    c.IsScheduleOK,
                    ScheduledAt = c.ScheduledAt.HasValue ? accessToken.ConvertTimeFromUtc(c.ScheduledAt.Value).ToString(Utilities.DateTime_Format_General_NO_Seconds) : null,
                    c.SentCounter,
                    c.DeliveryMethod,
                    c.Subject
                }).ToArray();
