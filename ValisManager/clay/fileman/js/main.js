      ...
      if(!RoxyFilemanConf.UPLOAD){
        ...
        //$('#dlgAddFile').dialog('close');
          ...
          if (typeof (FormData) == 'undefined') {
              alert('Is not supported by your browser! Please change browser!');
              return;
          }

          var formData = new FormData();
          formData.append('Executable', $('#fileUploads')[0].files[0]);

          var actionUrl = RoxyUtils.AddParam(RoxyFilemanConf.UPLOAD, 'scId', RoxyUtils.GetUrlParam('scId'));
          actionUrl = RoxyUtils.AddParam(actionUrl, 'survey', RoxyUtils.GetUrlParam('survey'));


          $.ajax({
              url: actionUrl, dataType: 'json', data: formData, type: 'POST', processData: false, contentType: false, cache: false,
...
function getSelectedFile(){
  ...
  if($('#pnlFileList .selected').length > 0)
      ret = new File($('#pnlFileList .selected').attr('data-path'), 0, 0, 0, 0, $('#pnlFileList .selected').attr('data-fileid'), $('#pnlFileList .selected').attr('data-url'));
  return ret;
}
function getSelectedDir(){
  return new Directory('survey', 0, 0);
}
...
function previewFile(){
  var f = getSelectedFile();
  if (f) {
      var url = RoxyUtils.AddParam(f.url, 'scId', RoxyUtils.GetUrlParam('scId'));
      window.open(url);
  }
}
function downloadFile(){
  ...
  if(f && RoxyFilemanConf.DOWNLOAD){
      ...
      url = RoxyUtils.AddParam(url, 'scId', RoxyUtils.GetUrlParam('scId'));
      url = RoxyUtils.AddParam(url, 'fileid', f.fileid);
...
function tooltipContent(){
  ...
  if($('#hdViewType').val() == 'thumb' && f.IsImage()){
    ...
    if(RoxyFilemanConf.GENERATETHUMB){
        ...
        imgUrl = RoxyUtils.AddParam(imgUrl, 'scId', RoxyUtils.GetUrlParam('scId'));
        imgUrl = RoxyUtils.AddParam(imgUrl, 'fileid', $(this).attr('data-fileid'));
...
$(function(){
  ...
  var d = new Directory('survey', 0, 0);
  d.Select();
  $('#wraper').show();
...
function setFile(){
  ...
  window.opener.CKEDITOR.tools.callFunction(RoxyUtils.GetUrlParam('CKEditorFuncNum'), f.url);
  self.close();
}