...
function Directory(fullPath, numDirs, numFiles){
  ...
  this.LoadFiles = function(refresh){
    ...
    fileURL = RoxyUtils.AddParam(fileURL, 'scId', RoxyUtils.GetUrlParam('scId'));
    fileURL = RoxyUtils.AddParam(fileURL, 'survey', RoxyUtils.GetUrlParam('survey'));
    var item = this;
      ...
      $.ajax({
          ...
          success: function(files){
              for(i = 0; i < files.length; i++){
                  ret.push(new File(files[i].p, files[i].s, files[i].t, files[i].w, files[i].h, files[i].fileid, files[i].url));
              }
